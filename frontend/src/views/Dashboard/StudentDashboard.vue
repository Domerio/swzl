...
export default {
  ...
  methods: {
    ...
    if (process.env.NODE_ENV === 'development') {
        ...
        this.recentRecords = this.mockChartData();
        return;
    }
    try {
        // 1. 获取统计信息
        ...
        this.statsData = [
            { title: '我的发布', value: stats.total_posts || 0, icon: 'el-icon-document', color: '#409EFF' },
            { title: '正在跟进', value: stats.active_posts || 0, icon: 'el-icon-s-order', color: '#67C23A' },
            { title: '已解决问题', value: stats.solved_posts || 0, icon: 'el-icon-success', color: '#E6A23C' },
            { title: '未读消息', value: stats.unread_messages || 0, icon: 'el-icon-message', color: '#F56C6C' }
        ];

        // 2. 获取最近记录
        const { data: records } = await this.$http.get('/api/records/recent/');
        if (records.status === 'error') throw new Error(records.message);
      this.recentRecords = records.data.map(item => ({
            id: item.id,
            title: item.title,
            item_type: item.item_type === 'lost' ? '丢失物品' : '捡到物品',
            status: item.status,
