...
def register(request):
    try:
        ...
        return Response(
            {"error": serializer.errors, "message": "注册数据验证失败"},
...
def user_logout(request):
    try:
        ...
        return Response({"code": 500, "message": "登出过程发生错误：" + str(e)})
...
def user_dashboard(request):
    try:
        ...
        daily_stats = (
            LostAndFound.objects.filter(user=user, created_at__gte=last_week)
        ...
        # 统计各种状态的信息数量
        status_stats = (
            LostAndFound.objects.filter(user=user)
            .values("status")
            .annotate(count=Count("id"))
        )
        # 获取物品分类统计
        ...
        return_data = {
            ...
            "data": {
                "recent_posts": [
                    {
                        ...
                        "item_type": (
                            post.category.item_type if post.category else None
                        ),  # 新增
                ...
                "category_stats": [
                    {"name": stat["category__name"] or "未分类", "value": stat["value"]}
                    for stat in category_stats
                ],
                "activity_dates": [
                    date.strftime("%Y-%m-%d") for date in activity_dates
                ],
...
def upload_avatar(request):
    try:
        return Response(
            {
                ...
                "data": {
                    "avatar_url": (
                        request.user.avatar.url if request.user.avatar else None
                    ),
                },
            }
        )
        ...
        return Response(
            {"error": "头像上传失败"}, status=status.HTTP_500_INTERNAL_SERVER_ERROR
        )
...
def mark_all_notifications_read(request):
    ...
    try:
        ...
        return Response({"message": "已将所有通知标记为已读"})
    except Exception as e:
...
def item_detail(request, item_id):
    try:
        ...
        return Response(
            {"error": "An error occurred while retrieving item details"},
            status=status.HTTP_500_INTERNAL_SERVER_ERROR,
        )
...
def admin_recent_posts(request):
    if request.user.role == "admin":
        ...
        return Response({"recent_posts": data})
    else:
        return Response(
            {"error": "You are not authorized to access this endpoint"},
            status=status.HTTP_403_FORBIDDEN,
...
def admin_recent_users(request):
    if request.user.role == "admin":
        ...
        return Response({"recent_users": data})
    else:
        return Response(
            {"error": "You are not authorized to access this endpoint"},
            status=status.HTTP_403_FORBIDDEN,
...
def admin_item_detail(request, item_id):
    try:
        item = (
            LostAndFound.objects.select_related("user")
...
def update_item_status(request, item_id):
    try:
        ...
        return Response({"status": "success", "data": LostItemSerializer(item).data})
...
def admin_approve_item(request, item_id):
    ...
    try:
        ...
        if item.status != "pending":
            return Response(
                {"error": "非待审状态物品无法操作"}, status=status.HTTP_400_BAD_REQUEST
            )
        item.status = "active"
        ...
        # 新增智能匹配逻辑
        from .services.matching import find_and_notify_matches
        find_and_notify_matches(item)  # 调用匹配服务
        ...
        return Response({"error": "目标物品不存在"}, status=status.HTTP_404_NOT_FOUND)
    except Exception as e:
...
def bookmarks_api(request, item_id):
    try:
        ...
        if request.method == "POST":
            ...
            return Response(
                {"message": "已取消收藏"}, status=status.HTTP_204_NO_CONTENT
            )
...
def report_found_and_notify(request, item_id):
    try:
        ...
        return Response(
            {"status": "success", "message": "已发送通知给失主"},
...
def report_lost_and_notify(request, item_id):
    try:
        ...
        return Response(
            {"status": "success", "message": "已发送通知给拾取者"},
            status=status.HTTP_200_OK,
        )
...
def public_found_items(request):
    ...
    try:
        queryset = (
            LostAndFound.objects.select_related("category")
            .filter(status="active", category__item_type="found")  # 修改为招领类型
            .order_by("-created_at")
        )
...
def public_lost_items(request):
    ...
    try:
        ...
        queryset = (
            LostAndFound.objects.select_related("category")
            .filter(status="active", category__item_type=item_type)
            .order_by("-created_at")
        )
        ...
        return Response({"error": "无法获取数据，请稍后再试"}, status=500)
...
def generate_admin_report(request):
    ...
    if not data:
        print(
            f"[{datetime.now()}] 空数据警告 - 报表类型: {report_type}"
        )  # 添加调试日志
        return Response({"error": "没有找到相关数据"}, status=404)

    print(
        f"样本数据: {json.dumps(data[:1], indent=2, ensure_ascii=False)}"
    )  # 现在json模块已定义
